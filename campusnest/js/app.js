...
function initChatGroups() {
  ...
  for (var i = 0; i < groups.length; i++) {
    var g = groups[i];
    html +=
      '<div class="group-item" onclick="selectGroup(' +
      g.id +
      ')">' +
      "<h4>" +
      g.name +
...
for (let d = 1; d <= lastDate; d++) {
  ...
  const dateStr = formatDateLocal(dateObj); // ✅ local-safe
  ...
  if (dayEvents.length === 1)
    markHtml =
      '<div class="events-mark"><span class="event-dot" title="1 event"></span></div>';
  else if (dayEvents.length > 1)
    markHtml = `<div class="events-mark"><span class="event-count" title="${dayEvents.length}">${dayEvents.length}</span></div>`;
...
function deleteEvent(id, date) {
  ...
  if (idx !== -1) {
    ...
    saveEvents();
    renderCalendar();
    renderEventsForSelected(date); // pass date to refresh properly
  }
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  try {
    ...
    /* ignore if calendar not present */
  }
});
  ...
  if (chatMessages) {
    ...
    setTimeout(function () {
      ...
      responseDiv.innerHTML =
        "<strong>Support:</strong> Thank you for your message!";
